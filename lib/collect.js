...
var findCodeBlock = require('./code-block/find');
var codeBlockAtIndent = require('./code-block/at-indent');
...
function getType(str){
  ...
  if(typeof str === 'undefined'){
    return constants.TYPES.COMMENT.label;
  }
  ...
  for(t in constants.TYPES){
    c = constants.TYPES[t];
    if(new RegExp(c.regex).test(str)){
      return c.label;
    }
  }
}
...
function findDependencies(code){
  ...
  if(constants.MIXIN_CALL_REGEX.test(code)){
    ...
    mixins.forEach(function(mixin){
      ...
      var dep = findCodeBlock('mixin '+ key, lines);
      if(dep){
...
function getComment(line, cursor){
  ...
  if(line.trim().indexOf(flag) === 0 && line.trim().length === flag.length){
    ...
    // get comment body 
    var comment = codeBlockAtIndent(cursor, indent, lines);
    ...
    if(root){
      ...
      var code = [root];
      code.push(codeBlockAtIndent(comment.cursor, indent, lines).body);
      obj.code = code.join('\n');
