 ...
 * This is needed because in jade 'data-foo'
 * is a valid attribute, but that's not a 
 * valid jsdoc name.
 * https://github.com/Aratramba/jade-doc/issues/39 
 */
...
function escapeArgumentsYAML(str, needle){
  ...
  var escapedArgsBlock = argsBlock.replace(/([\s].-[\s+]?)(.*)/gi, function(match, p1, p2){
    return [p1, '"', p2, '"'].join('');
  });
