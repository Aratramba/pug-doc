...
function getPugdocDocuments(templateSrc, filename){
  return extractPugdocBlocks(templateSrc)
    .map(function(pugdocBlock) {
      ...
      return {
        ...
        source: pugdocBlock.code,
        // get html output
        output: compilePug(pugdocBlock.code, meta, filename)
      };
    });
}
...
function compilePug(src, meta, filename){
  ...
  // compile pug
  var compiled = pug.compileClientWithDependenciesTracked(src, { filename: filename });
  // render pug function
  return Function('pug', compiled.body + '\n' +'return template('+ JSON.stringify(meta.locals || {}) +');')(pugRuntime);
}
