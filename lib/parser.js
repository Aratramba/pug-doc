...
function extractPugdocBlocks(templateSrc, options) {
  return (
    templateSrc
      .split("\n")
      ...
      .map(function (line, lineIndex) {
        ...
        // try and find the name of the component
        const findName = /name\s*:\s*(.*)/gi.exec(templateSrc);
        if (findName && options && options.log) {
          options.log(`\u001B[32m${findName[1]}\u001B[39m`, "processing");
        }
        ...
        try {
          ...
          // pug block contains an error
          if (options && options.log) {
            options.log(`\u001B[31m${err}\u001B[39m\n`, null, "");
          }
        }

        if (!comment) return undefined;

        const meta = parsePugdocComment(comment, options);
        ...
        let code = null;
        try {
          code = getCodeBlock.byLine(templateSrc, lineIndex + 1, capture);
        } catch (err) {
          // pug block contains an error
          if (options && options.log) {
            options.log(`\u001B[31m${err}\u001B[39m\n`, null, "");
...
function getPugdocDocuments(
  ...
  return extractPugdocBlocks(templateSrc, options).map(function (pugdocBlock) {
    if (!pugdocBlock) return;

    const meta = parsePugdocComment(pugdocBlock.comment, options);

    const fragments = [];

    // see if complete cache can be used
    ...
    if (documentCache) {
      ...
      if (pugdocBlock.comment !== documentCache.comment) {
        if (options && options.log) {
          options.log(`overwriting ${meta.name} (pugdoc changed)`);
        }
        useCache = false;
      }
      ...
      if (pugdocBlock.code !== documentCache.source) {
        if (options && options.log) {
          options.log(`overwriting ${meta.name} (pug changed)`);
        }
        useCache = false;
      }
    ...
    const obj = {
      ...
      source: source,
      // get html output
      output: compilePug(source, meta, filename, locals, options),
    ...
    if (fragments && fragments.length) {
      obj.fragments = fragments.map((subexample) => {
        return {
          ...
          meta: subexample,
          // get html output
          output: compilePug(source, subexample, filename, locals, options),
        };
      });
    }
...
function parsePugdocComment(comment, options) {
  ...
  let parsedYAML = {};
  try {
    parsedYAML = YAML.safeLoad(comment) || {};
  } catch (err) {
    if (options && options.log) {
      // log yaml error
      options.log(`\u001B[31mYAML Error: ${err.message}\u001B[39m`, null, "");

      // highlight line where the error occurred
      const errorLog = err.mark.buffer.split("\n");
      errorLog[err.mark.line] = `\u001B[1m${errorLog[err.mark.line]}\u001B[22m`;
      options.log(`\u001B[31m${errorLog.join("\n")}\u001B[39m`, null, "");
    }
    return {};
  }
  return parsedYAML;
...
function compilePug(src, meta, filename, locals, options) {
  let newSrc = [src];
  ...
  try {
    ...
    try {
      ...
      if (options && options.log) {
        options.log(`error: ${JSON.stringify(meta, null, 2)}`, null, "");
        options.log(`\n\u001B[31m${debugErr.toString()}\u001B[39m\n`, null, "");
      }
      return null;
    }
  }
